...
3. If more than one candidate task could fill the same role (e.g. two tasks that both take fastq and produce bam), pick exactly one — prefer the one whose `description` best matches the user's requested operations.
4. Ensure all operations are covered by selected tasks, if possible.
5. Build one chain of tasks, matching one task's `output_sample`/`output_reference` to another's `input_sample_required`/`input_reference_required` by `category:format`.
6. If an operation can't be reached from the user's input data through any chain, drop it and continue with what's possible.
7. If one task's output feeds more than one downstream task (e.g. one alignment task feeding both a variant caller and a copy-number caller), list it once and list each downstream task separately.
